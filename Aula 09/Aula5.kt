...
/*
    ...
    3. Classes abstratas podem possuir propriedades e métodos com implementações e valores definidos. Interfaces não podem armazenar estado, pois não possuem campos próprios.
*/
...
class Calculos {
    ...
    fun somar(numero1: Int, numero2: Int, numero3: Int): Int {
        return numero1+numero2+numero3
    }
}

// POLIMORFISMO - SOBRESCRITA (OVERRIDE)
open class DescontoPadrao {
    fun valeTransporte(salario: Double): Unit {
        println("Vale Transporte padrão R$ ${salario * 0.06}")
    }

    open fun impostoDeRenda(salario: Double): Unit {
        println("Imposto de Renda padrão R$ ${salario * 0.1}")
    }
}

class DescontoDesenvolvedor : DescontoPadrao() {
    override fun impostoDeRenda(salario: Double): Unit {
        println("Imposto de Renda desenvolvedor R$ ${salario * 0.15}")
    }
}
...
fun main() {
    ...
    val colaborador: DescontoPadrao = DescontoDesenvolvedor()
    colaborador.valeTransporte(1000.0)
    colaborador.impostoDeRenda(1000.0)
